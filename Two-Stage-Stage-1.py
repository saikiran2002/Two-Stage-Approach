...
# Model
def construct_alexnet_model(act='relu'):
    model = Sequential([
    ...
    ], name = "alexnet_cnn_model")

    return model
opt = tf.keras.optimizers.Adam()
model = construct_alexnet_model()
...
# Training
EPOCHS = 100
history = model.fit(train_data_gen, validation_data=(val_data, val_labels), callbacks=CALLBACKS, epochs=EPOCHS)
...
ut.Confusion_matrix(title="Stage-1 Classification")

ut.balanced_accuracy_score()
ut.Mathews_correlation_coefficient()

model_dir = "./models/" + "Stage_1_cnn_model"
